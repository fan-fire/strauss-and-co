function getContractFunctions() {
    ...
    abi.forEach((element) => {
      ...
      if (
        ...
        let prettyFunction = `${element.name}(${element.inputs.map((input) => `${input.type} ${input.name}`).join(', ')})`;
        prettyFunction += element.outputs.length > 0 ? ` returns (${element.outputs.map((output) => `${output.type} ${output.name}`).join(', ')})` : '';
